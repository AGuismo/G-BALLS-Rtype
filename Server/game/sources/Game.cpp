...
namespace game
{
  ...
  Game::Game(const client_list &players):
    _gameID(0), _parent(0), _status(WAITING_PLAYER), _referee(0)
  {
    _clients = players;
    _timer.tv_sec = 0;
  ...
  //     {
  // 	pushRequest(new DeathRequest((*itia)->id(), 0)); // STAMP !
  // 	delete *itia;
  // 	itia = _IA.erase(itia);
  // 	flag = true;
  //     }
  ...
  //     {
  // 	pushRequest(new DeathRequest((*ite)->id(), 0)); // STAMP !
  // 	delete *ite;
  // 	ite = _objs.erase(ite);
  // 	flag = true;
  //     }
  ...
  // 	{
  // 	  pushRequest(new DeathRequest((*itm)->id(), 0)); // STAMP !
  // 	  delete *itm;
  ...
  //     {
  // 	pushRequest(new DeathRequest((*itb)->id(), 0)); // STAMP !
  // 	delete *itb;
  ...
  //     {
  // 	pushRequest(new DeathRequest(_titan->id(), 0)); // STAMP !
  // 	if (_titans.empty())
  // 	  _status = FINISHED;
  ...
  void	Game::sendUpdates(const std::vector<unsigned short> &toDelete)
  {
    const MainReferee::entity_set_type	&map = _referee->getMap();
    ...
    // for (client_list::const_iterator it = _clients.begin(); it != _clients.end(); it++)
    // {
    //   send(ElemRequest::create<Player>(*((*it)->player()), _gameID, getStamp()));
    // }
