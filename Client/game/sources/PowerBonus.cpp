...
void			PowerBonus::update(game::Dir lDir, int updtatedPos)
{
	(void)lDir;
...
PowerBonus::PowerBonus(game::Type type, int id, int pos, game::Dir ld, sf::Texture *text, sf::RenderWindow *gameWindow)
{
	_type = type;
	_id = id;
	_texture = text;
	...
	_image.setTextureRect(sf::IntRect(0, 0, 76, 52));
	_image.setPosition((float)Game::POSX(_cCurPos), (float)Game::POSY(_cCurPos));
	_mvtTime = 0.60f;
