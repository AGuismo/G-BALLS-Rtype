...
MenuWindow::MenuWindow(sf::RenderWindow &window) :
  AScreen(window, START), _objectFocus(0), _objectHover(0)
{
  this->_flag = 0;
...
int	MenuWindow::checkAction()
{
  ...
  case AScreen::START:
    ...
    break;
  case AScreen::GAME:
    network::Manager::getInstance().sendRequest(Party::Start(), network::Manager::TCP);
    this->_status = CONTINUE;
    break;
  case AScreen::LAUNCH:
    this->clearWindow();
  ...
  case AScreen::WAIT:
    this->_status = CONTINUE;
    network::Manager::getInstance().sendRequest(Party::Join(this->_serverSelected._name, dynamic_cast<Text*>(Interface::getInstance().getWidget("setPWD"))->getTmp()), network::Manager::TCP);
    break;
  case AScreen::LOGIN:
    this->_status = CONTINUE;
    ...
    if (!network::Manager::getInstance().setTcp(sf::IpAddress(InfosUser::getInstance().authenticate.addressIp), InfosUser::getInstance().authenticate.portTCP))
    {
      this->drawMenuWarning("Unable to reach the server !");
      break;
    }
    if (network::Manager::getInstance().isConnected(network::Manager::TCP))
    {
      network::Manager::getInstance().sendRequest(Auth::Connect(InfosUser::getInstance().authenticate.login, md5(InfosUser::getInstance().authenticate.password)), network::Manager::TCP);
      break;
    }
    else
    {
  ...
  case AScreen::BACK_MENU:
    network::Manager::getInstance().closeTcp();
    this->_isConnected = 0;
    this->_status = CONTINUE;
    this->drawMenu();
    break;
  ...
  case AScreen::REFRESH_GAME:
    network::Manager::getInstance().sendRequest(Party::List(), network::Manager::TCP);
    this->_status = CONTINUE;
    break;
  case AScreen::JOIN_GAME:
    ...
    if (this->_serverSelected._lock)
      ...
      network::Manager::getInstance().sendRequest(Party::Join(this->_serverSelected._name), network::Manager::TCP);
    break;
  case AScreen::VALIDE:
    ...
    if (InfosUser::getInstance().game.partyPassword.empty() || InfosUser::getInstance().game.partyPassword == "Party Password")
      network::Manager::getInstance().sendRequest(Party::Create(InfosUser::getInstance().game.partyName, this->checkNbPlayer()), network::Manager::TCP);
    else
      network::Manager::getInstance().sendRequest(Party::Create(InfosUser::getInstance().game.partyName, this->checkNbPlayer(), md5(InfosUser::getInstance().game.partyPassword)), network::Manager::TCP);
    this->_status = CONTINUE;
    break;
  case AScreen::SUBMIT:
    this->_status = CONTINUE;
    if (dynamic_cast<Text*>(Interface::getInstance().getWidget("MsgChat"))->getTmp() != "")
    {
      ...
      network::Manager::getInstance().sendRequest(ChatSendRequest(msgChat), network::Manager::TCP);
    }
  ...
  case AScreen::LEAVE_GAME:
    this->_status = AScreen::CONTINUE;
    network::Manager::getInstance().sendRequest(Party::Cancel(), network::Manager::TCP);
    break;
...
void	MenuWindow::update()
{
  if (network::Manager::getInstance().isConnected(network::Manager::TCP))
  {
    ...
    while ((req = network::Manager::getInstance().recvRequest()) != 0)
    {
  ...
  else if (!network::Manager::getInstance().isConnected(network::Manager::TCP) && this->_isConnected == 1)
  {
